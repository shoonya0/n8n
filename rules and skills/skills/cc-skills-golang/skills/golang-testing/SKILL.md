...
## dm-enquer-go: Java vs Go parity (MySQL)

This repo maintains production parity in MySQL (`parity_test_cases`). **Do not** drive ingestion from ad-hoc shell pulls alone — use the scripted pipeline:

1. `go run ./scripts/pubsub-pull/` — subscription → NDJSON file  
2. `go run ./scripts/log-ingest/` — `-kind java|go` → MySQL columns  
3. `go run ./scripts/replay-tester/` — replay inputs to Cloud Run (`requestid`=baseline req id)  
4. `go run ./scripts/parity-compare/` — writes `diff_json` + `status`

→ Full workflow and troubleshooting: `.cursor/skills/parity-testing/SKILL.md` (`parity-testing` skill).
